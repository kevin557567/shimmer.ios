...
struct server_config {
	...
	uint8_t trace_level;
	bool extend;       //enable extend protocol
	char *passwords[8];
	char *storage;
	bool innov_auth; //innov auth
	char *database;
	char *auth_host;
    bool tcp_usock;
...
struct server_env_t {
	...
	//edit by caozhenbiao
	struct cipher_env_t * innov_cipher;
	struct cipher_env_t *ciphers[ss_cipher_max][8];
	void * protocol_globals[ssr_protocol_max];
	void * obfs_globals[ssr_obfs_max];
	uint8_t token[0x11];  //client store
	bool auth_vaild;
	bool sync_done;
...
struct tunnel_cipher_ctx {
	...
	struct cipher_env_t * chiper;   // tunnel context
};
...
struct server_env_t * ssr_client_cipher_env_create(struct server_config *config, void *data);
struct server_env_t * ssr_server_cipher_env_create(struct server_config *config, void *data);
...
struct tunnel_cipher_ctx * tunnel_server_cipher_create(int imet, int ipwd, int ipro, int iobs, struct server_env_t *env, size_t tcp_mss);
